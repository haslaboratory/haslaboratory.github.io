...
tags:
 ...
 - consensus protocol
---
### 《CRaft: An Erasure-coding-supported Version of Raft for Reducing Storage Cost and Network Cost》论文阅读笔记

**文章：**CRaft: An Erasure-coding-supported Version of Raft for Reducing Storage Cost and Network Cost

**出处：**FAST '20

**作者：**Zizhong Wang, Tongliang Li, Haixia Wang, Airan Shao, Yunren Bai,  Shangming Cai, Zihan Xu, Dongsheng Wang

**机构：**Tsinghua University

**原文地址：**https://www.usenix.org/conference/fast20/presentation/wang-zizhong

#### 1.背景

（1）传统的共识协议（如Raft）在分布式服务中提供了高可靠性和可用性。 但在这些协议中，日志条目会被完全复制到所有服务器。 这种完整的复制会增加存储和网络成本，并对性能产生不利影响。

（2）纠删码（erasure code）技术是一种在保持容错能力的同时降低存储和网络成本的常用技术。如果用erasure code代替日志条目的完全复制，则可以大大降低存储和网络成本。在此前，已经有RS-Paxos共识协议将其应用到Paxos协议中。但在RS-Paxos共识协议中存在活性的问题。 

（3）在此基础上，作者提出了一种新的分布式共识协议CRaft。CRaft保持了和Raft相同的活性，同时将erasure code技术应用到Raft协议中，降低了存储和网络成本。

#### 2.Erasure code原理

https://blog.csdn.net/sinat_27186785/article/details/52034588

https://zhuanlan.zhihu.com/p/106096265?utm_source=wechat_session
...
![image-20201103110302535](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201103110302535.png)



比如：我们有 7、8、9 三个原始数据，通过矩阵乘法，计算出来两个校验数据 50、122。这时原始数据加上校验数据，一共五个数据：7、8、9、50、122，可以任意丢两个，然后通过算法进行恢复。

![image-20201103110315787](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201103110315787.png)
...
![image-20201125195008112](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201125195008112.png)
...
![image-20201125193512782](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201125193512782.png)

​	在吞吐量实验中，为了避免活性问题，在进行比较时应选择同等Value Size的5台服务器下的CRaft（左图示意）和7台服务器下的RS-Raft（右图示意）比较吞吐量。

**活性实验比较结果：**

![image-20201125193909331](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201125193909331.png)

**延迟实验比较结果：**

![image-20201125193933760](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20201125193933760.png)
